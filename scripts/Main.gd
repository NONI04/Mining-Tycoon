...
var _pan_accumulator: float = 0.0
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventPanGesture:
		if get_viewport().get_mouse_position().x < 760.0:
			_pan_accumulator += event.delta.y
			while _pan_accumulator >= 30.0:
				_camera.position.y = clamp(
					_camera.position.y + SCROLL_SPEED, 360.0, _camera_max_y())
				_pan_accumulator -= 30.0
			while _pan_accumulator <= -30.0:
				_camera.position.y = clamp(
					_camera.position.y - SCROLL_SPEED, 360.0, _camera_max_y())
				_pan_accumulator += 30.0
...
func _build_ui() -> void:
	...
	_money_label.add_theme_font_size_override("font_size", 21)
	...
	storage_title.add_theme_font_size_override("font_size", 27)
